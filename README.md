...
- **税込経理方式における納付消費税の必要経費算入は未実装**。デモ・エンジンは売上・経費を税込のまま P/L に計上する税込経理を採っていますが、税込経理では本来、確定した納付消費税額を**租税公課として必要経費に算入する決算整理**（未払計上または支払時計上）が伴います。消費税エンジンは納付額を算定するものの、その結果を所得計算へ還流する経路は未実装です。所得金額に影響するため、税抜経理を採るか、この決算整理を別途行う必要があります。
