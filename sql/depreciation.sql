...
--
--  網羅範囲について:
--    - 定額法は別表第八の 2〜20年を全て収録(定額法の率は別表で確定値。要突合)。
--    - 定率法(200%)は主要耐用年数のみを収録。保証率・改定償却率は別表からの転記が
--      必要で、誤りは申告に直結するため、未確認の年数はあえて投入していない。
...
COMMENT ON TABLE depreciation_rates IS
  '耐用年数省令の別表(定額法=別表第八/200%定率法=別表第十)。償却率等をコードから外出ししたもの。定額法は2〜20年を収録、定率法は主要年数のみ。導入時に国税庁の償却率等表と要突合。';
...
-- 200%定率法(別表第十)  ※主要耐用年数のみ。未収録年数は別表から確認のうえ追記すること。
